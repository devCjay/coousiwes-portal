...
<x-layouts.app-shell title="Assessment Rubric" role="Admin" :navigation="$navigation">
    ...
    <div class="mt-6 grid gap-6 {{ $canUpdateSettings ? 'xl:grid-cols-[0.85fr_1.15fr]' : '' }}">
        ...
        <x-ui.card title="Rubric Library" description="Live-search and update the scoring model without code changes.">
            @php
                $rubricHeaders = $canUpdateSettings
                    ? ['Item', 'Score', 'Weight', 'Status', 'Update']
                    : ['Item', 'Score', 'Weight', 'Status'];
                $rubricRows = $rubricItems->map(function ($item) use ($canUpdateSettings) {
                    $row = [
