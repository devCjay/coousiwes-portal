@props(['title' => 'Sign in', 'role' => null, 'centered' => false])

<x-layouts.public :title="$title">
    <x-layouts.public-header />

    <main class="relative grid min-h-[calc(100vh-10.5rem)] place-items-center px-4 py-10">
        <section data-reveal="scale" @class([
            'w-full overflow-hidden rounded-lg siwes-surface',
            'max-w-5xl grid lg:grid-cols-[1fr_0.88fr]' => ! $centered,
            'max-w-xl' => $centered,
        ])>
            @unless ($centered)
            ...
            <div @class([
                'bg-[var(--surface-raised)] p-6 md:p-8 lg:p-10',
                'p-6 sm:p-8' => $centered,
            ])>
                {{ $slot }}
            </div>
