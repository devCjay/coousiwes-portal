...
class UpdateProfileStepRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $hasProfilePhoto = filled($this->user()?->metadata['profile_photo_path'] ?? null);

        return match ((string) $this->input('step')) {
            'basic' => [
                'step' => ['required', Rule::in(['basic'])],
                'profile_photo' => [$hasProfilePhoto ? 'nullable' : 'required', 'image', 'mimes:jpg,jpeg,png,webp', 'max:2048'],
